...
const MessageList = ({ messages, loading }) => {
    // Convert any content to a safe string
            ...
            {messages.map((msg, idx) => (
                ...
                >
                    <div className={`avatar placeholder rounded-full w-8 h-8 flex items-center justify-center text-white ${msg.role === "user" ? "bg-purple-700" : "bg-purple-500"}`}>
                        {msg.role === "user" ? <FaUser size={14} /> : <img src={robot} alt="robot" />}
                    </div>
                    <div
                    ...
                    >
                        {msg.role === "user" ? (
                            ...
                            <ReactMarkdown remarkPlugins={[remarkGfm]}>{asString(msg.content)}</ReactMarkdown>
                        )}
                    </div>
                </div>
            ))}
